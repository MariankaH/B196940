...
To account for changes in the ratios over time, I calculated the difference for each Health Board by subtracting the 2019 ratios from the 2023 ratios. This approach highlights the magnitude of change in prescribed doses relative to population across the two time periods.

```{r}
difference_plot <- map_data %>%
  ungroup() %>%
  ...
  mutate(differences = ratio_2023 - ratio_2019) %>% 
  ggplot(aes(x = hb_name, y = differences, text = paste(hb_name, "has a difference of", format(differences, digits = 2)))) +
  geom_col() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(title = "Bar Plot Showing Differences in Ratio of Prescribed Doses of ADHD Medications per Health Board Populations") +
  xlab("Health Board Names") +
  ylab("Differences")
